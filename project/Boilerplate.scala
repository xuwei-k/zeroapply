object Boilerplate {
  ...
  private final class Gen(i: Int) {
    ...
    val paramsF = a.lazyZip(t).map((x, y) => x + ": F[" + y + "]").mkString(", ")
    val inlineParamsF = a.lazyZip(t).map((x, y) => s"${x}: F[${y}]").mkString(", inline ")
    val paramsFL = a.lazyZip(t).map((x, y) => x + ": F[L, " + y + "]").mkString(", ")
    val inlineParamsFL = a.lazyZip(t).map((x, y) => s"${x}: F[L, ${y}]").mkString(", inline ")
